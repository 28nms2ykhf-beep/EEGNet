...
class EEGDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            return torch.zeros(1, 18, 1536), -1, patient, session 
...
def random_center_crop(eeg, crop_length=1536, center_idx = 2048):
    start = torch.randint(513, 2049, (1,)).item()
...
def build_augmented_samples(base_samples, seizure_target, num_augmented, seed=42):
    """
    Augmentation data are based on orginal data, created a external augumentation set
    base_samples --> orginal training set
    seizure_target--> seizure samples for resampling
    num_augmented --> total augmented data has been generated
    """
    import random
    random.seed(seed)
    torch.manual_seed(seed)

    # split seizure and non-seizure
    seizure_samples = [s for s in base_samples if s[4] == 1]
    n_seizure = len(seizure_samples)
    n_duplicate = max(0, seizure_target - n_seizure)

    # resampple the sz samples
    duplicated_seizure = []
    if n_duplicate > 0 and n_seizure > 0:
        choices = random.choices(seizure_samples, k=n_duplicate)  
        for sample in choices:
            duplicated_seizure.append(sample[:5] + (None,))
    ...
    # transfer orginal training set to non aug label
